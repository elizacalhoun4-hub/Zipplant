...
with tabs[0]:
    ...
    if st.session_state.weather:
        ...
        <div class="granny-box">
            ...
            <span style="color:#9be28f; font-size:1.2em;">{zone}</span>.<br>
        ...
        for i in range(7):
            ...
            with st.expander(
                ...
                with cols[0]:
                    st.metric("🌡️ Low/High", f"{tmin}° / {tmax}°F")

                with cols[1]:
                    st.metric("🌧️ Rain", f"{rain_prob}% ({rain_in:.2f} in)")

                with cols[2]:
                    st.metric("☀️ UV Max", str(uv))

                with cols[3]:
                    st.metric("💧 ET₀", f"{et0:.2f} in")
                ...
                st.progress(
                    thirst,
                    text=(
                        f"Thirst Meter: {thirst}% — "
                        f"{'Water deeply today' if thirst > 60 else 'Normal watering'}"
                    )
                )

                if uv >= 8:
                    st.warning(
                        "☀️ **Granny says:** Strong sun today! "
                        "Don't leave your indoor babies out too long!"
                    )

        total_rain = sum(w["precipitation_sum"])

        avg_et0 = (
            sum(w.get("et0_fao_evapotranspiration", [0] * 7)) / 7
        )
...
with tabs[1]:
    ...
    with veg_tab:
        ...
        vegetables = [
            p for p in plants
            if p.get("category", "").lower() == "vegetable"
        ]
        ...
        for idx, p in enumerate(vegetables):
    ...
    with flower_tab:
        ...
        flowers = [
            p for p in plants
            if p.get("category", "").lower() == "flower"
        ]
        ...
        for idx, p in enumerate(flowers):
        ...
        for i, p in enumerate(st.session_state.garden[:]):
            ...
            with st.expander(
                ...
                with col_a:
                    ...
                    if st.checkbox(
                        "Show Grid Spacing (plants per sq ft)",
                        key=f"grid_{i}"
                    ):
                        st.write(
                            f"**Grid Mode:** "
                            f"{p.get('grid_spacing', 'N/A')} "
                            f"plants per square foot"
                        )
                    else:
                        st.write(
                            f"**Traditional Spacing:** "
                            f"{p.get('spacing', 'N/A')}"
                        )

                    st.write(
                        f"**Sowing Depth:** "
                        f"{p.get('sowing_depth_desc', 'N/A')}"
                    )
