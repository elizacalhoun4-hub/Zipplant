...
<style>
    ...
    .stButton>button { background: #1f6f4a; color: white; border-radius: 8px; }
...
# ==================== BUILD GARDEN TAB - NEW SEARCH + FILTERS ====================
with tabs[1]:
    ...
    with col_filter1:
        category = st.selectbox("Category", ["All", "Vegetables", "Herbs", "Greens", "Roots", "Peppers", "Tomatoes"])
    with col_filter2:
    ...
    if search_term:
        filtered_plants = [p for p in filtered_plants if search_term.lower() in p["name"].lower() or search_term.lower() in p.get("type","").lower()]
    if category != "All":
        filtered_plants = [p for p in filtered_plants if category.lower() in p.get("type","").lower() or category.lower() in p["name"].lower()]
    if sun_pref != "Any":
        filtered_plants = [p for p in filtered_plants if sun_pref.lower() in p["sun"].lower()]
    ...
    if not filtered_plants:
        ...
        cols = st.columns(2)
        for idx, p in enumerate(filtered_plants):
            with cols[idx % 2]:
                with st.container():
                    st.markdown(f"""
                    <div class="plant-card">
                        <h3>{p['name']}</h3>
                        <small><em>{p.get('type', '')}</em></small><br>
                        <em>{p.get('pitch', '')}</em>
                    </div>
                    """, unsafe_allow_html=True)
