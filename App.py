...
with main_tabs[0]:
    ...
    if st.session_state.weather:
        ...
        # Forecast Expander
        for i in range(7):
            with st.expander(f"📅 {format_date(w['time'][i])}"):
                st.write(f"Low: {w['temperature_2m_min'][i]}°F | Rain: {w['precipitation_probability_max'][i]}%")

# ==================== BUILD GARDEN TAB (WITH SUB-TABS) ====================
with main_tabs[1]:
...
with main_tabs[2]:
    ...
    for i, p in enumerate(st.session_state.garden):
        with st.expander(f"🌱 {p.get('common_name')} - {p.get('cultivar')}", expanded=True):
            st.markdown(f"**🌾 Granny Fanny says:** *{p.get('granny_fanny', 'Keep it watered!')}*")
            st.write("---")
            
            # Field Guide Style vertical list
            fields = [
                ("Scientific Name", p.get('scientific_name')), ("Cultivar", p.get('cultivar')),
                ("Type", p.get('type')), ("Growth Habit", p.get('growth_habit')),
                ("Method", p.get('method')), ("Depth", p.get('depth')),
                ("Spacing (Traditional)", p.get('spacing_traditional')), ("Spacing (Grid)", p.get('spacing_grid')),
                ("Care", p.get('care')), ("Days to Harvest", p.get('days_to_harvest')),
                ("Harvest Indicators", p.get('harvest_indicators')), ("Yield for Couple", p.get('yield_family_2')),
                ("Yield for Family of 4", p.get('yield_family_4')), ("When to Pick", p.get('when_to_pick')),
                ("Preservation", p.get('preservation')), ("Companions", p.get('companions')),
                ("No-Go", p.get('no_go'))
            ]
