...
with tabs[0]:
    ...
    if st.session_state.weather:
        ...
        st.info("More weather details coming soon...")
...
with tabs[1]:
    ...
    with col3:
        ptype = st.selectbox("Type", ["Any", "Heirloom", "Hybrid"])
    ...
    filtered = [p for p in plants if 
                ...
                (ptype == "Any" or ptype.lower() == p.get("heirloom_hybrid", "").lower())]

    cols = st.columns(2)
    for idx, p in enumerate(filtered):
        with cols[idx % 2]:
            st.markdown(f"<div class='plant-card'><h3>{p['name']}</h3><small>{p['type']} • {p.get('heirloom_hybrid','')}</small></div>", unsafe_allow_html=True)
            st.write(f"**Zones:** {p.get('recommended_zones', '5-9')}")
