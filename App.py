...
# ✅ FIXED: Fahrenheit explicitly requested
def get_weather(lat, lon):
    try:
        ...
        temps = r["daily"]["temperature_2m_min"][:10]
        return [round(t) for t in temps]  # clean rounding
...
def faux_ai(garden, temps):
    ...
    if not garden:
        return "Start by adding plants 🌿"

    if temps:
        if min(temps[:3]) < 35:
            advice.append("Cold snap coming — wait before planting.")
        elif min(temps[:3]) > 50:
            advice.append("Warm nights — good planting window.")

    names = [p["name"] for p in garden]

    if "Tomato" in names and min(temps[:5]) < 45:
        advice.append("Tomatoes may struggle — hold off a bit.")
...
def moon_phase():
    ...
    if day < 7: return "🌑 New Moon — leafy crops"
    elif day < 15: return "🌓 Growing Moon — above-ground plants"
    elif day < 22: return "🌕 Full Moon — strong growth"
    else: return "🌗 Waning Moon — root crops"
...
with tabs[0]:
    ...
    if "temps" in st.session_state:
        st.subheader("🌤 Next 10 Nights (Low Temps)")
...
with tabs[3]:
    ...
    if "temps" in st.session_state:
        advice = faux_ai(st.session_state.garden, st.session_state.temps)
        st.success(advice)
