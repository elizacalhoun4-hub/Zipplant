    ...
    if not st.session_state.garden:
        ...
        for i, p in enumerate(st.session_state.garden[:]):
            with st.expander(f"🌱 {p['name']}", expanded=True):
                ...
                with col_a:
                    if st.session_state.weather:
                        st.markdown(f"<span class='success-msg'>Best day: {best_day(st.session_state.weather)}</span>", unsafe_allow_html=True)
                    for key in ["spacing", "couple", "family4", "start", "care", "harvest_window", "yield"]:
                        if key in p:
                            st.write(f"**{key.replace('_',' ').title()}:** {p[key]}")
