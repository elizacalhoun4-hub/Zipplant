...
with tabs[0]:
    ...
    if st.session_state.weather:
        ...
        for i in range(7):
            ...
            with st.container():
                st.markdown(f"""
                <div style="background:#161b22; padding:1rem; border-radius:12px; margin-bottom:0.8rem; border:1px solid #2a3a2f;">
                    <strong>{date}</strong>
                </div>
                """, unsafe_allow_html=True)
                ...
                with col_t:
                    st.write(f"🌡️ **{temp}°F**")
                with col_r:
                    st.write(f"🌧️ Rain chance: **{rain}%**")
                
                if rain > 70:
                    st.warning("High rain chance")
                elif rain > 40:
                    st.info("Moderate rain chance")
...
with tabs[2]:
    if not st.session_state.garden:
        st.info("Your garden is empty. Go to Build Garden to add plants!")
    else:
