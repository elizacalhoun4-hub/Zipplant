    ...
    if st.session_state.weather:
        ...
        for i in range(7):
            ...
            with st.expander(f"**{date}**", expanded=(i==0)):
                ...
                st.write(f"UV: {w.get('uv_index_max', [0]*7)[i]} | ET₀: {w.get('et0_fao_evapotranspiration', [0]*7)[i]:.2f} in")
...
with tabs[1]:
    ...
    for idx, p in enumerate(filtered):
        with cols[idx % 2]:
            ...
            """, unsafe_allow_html=True)
            st.write(f"**Spacing:** {p.get('spacing', 'N/A')}")
            st.write(f"**Sun:** {p.get('sun', 'N/A')}")
