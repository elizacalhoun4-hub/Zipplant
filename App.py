...
def soil_readiness(temp_f):
    if temp_f < 50: return "🔴 Too chilly! Wait until it hits 60°F.", "Soil is still sleeping."
    elif temp_f < 60: return "🟡 Marginal", "Good for cool crops."
    else: return "🟢 Warm enough!", "Roots will wake up and grow strong."
...
def get_alerts(weather):
    ...
    for i in range(min(7, len(weather["time"]))):
        ...
        if tmin <= 36: alerts.append(f"❄️ Frost Risk on {date} ({tmin}°F)")
        if tmax >= 90: alerts.append(f"🔥 Heat Stress on {date} ({tmax}°F)")
        if wind >= 15: alerts.append(f"💨 Strong Wind on {date} ({wind} mph)")
    return alerts
...
# ==================== LOCATION TAB (The one you liked) ====================
with tabs[0]:
    ...
    if st.session_state.weather:
        ...
        for i in range(7):
            ...
            with st.expander(f"**{date}**", expanded=(i==0)):
                st.write(f"Low: {w['temperature_2m_min'][i]}°F | Rain: {w['precipitation_probability_max'][i]}% ({w['precipitation_sum'][i]:.2f} in)")
                st.write(f"UV Max: {w.get('uv_index_max', [0]*7)[i]} | ET₀: {w.get('et0_fao_evapotranspiration', [0]*7)[i]:.2f} in")
...
with tabs[1]:
    ...
    filtered = [p for p in plants if (not search_term or search_term.lower() in p["name"].lower()) and (category == "All" or category.lower() in p["type"].lower()) and (sun_pref == "Any" or sun_pref.lower() in p["sun"].lower())]

    cols = st.columns(2)
    for idx, p in enumerate(filtered):
        with cols[idx % 2]:
            st.markdown(f"<div class='plant-card'><h3>{p['name']}</h3><em>{p['pitch']}</em></div>", unsafe_allow_html=True)
