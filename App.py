...
def get_alerts(weather):
    ...
    for i in range(min(7, len(weather["time"]))):
        ...
        if tmax >= 90:
            alerts.append(f"🔥 Heat Stress on {date} ({tmax}°F) — tomatoes may stop setting fruit")
        if wind >= 15:
            alerts.append(f"💨 Strong Wind on {date} ({wind} mph) — secure cages")
    return alerts
...
with tabs[0]:
    ...
    if st.session_state.weather:
        ...
        <div class="granny-box">
            ...
            That's a wonderful zone for growing delicious things, sweetie.
        </div>
        """, unsafe_allow_html=True)
        ...
        for i in range(7):
            ...
            with st.expander(f"**{date}** — {tmin}°F to {tmax}°F", expanded=(i==0)):
                ...
                st.write(f"**Daylight:** {sunrise} — {sunset} | **Wind:** {wind} mph")
                ...
                if uv >= 8:
                    st.warning("☀️ **Granny says:** Strong sun today! Harden off indoor seedlings slowly.")

        # Week Summary
        total_rain = sum(w['precipitation_sum'])
        avg_et0 = sum(w.get('et0_fao_evapotranspiration', [0]*7)) / 7
        st.info(f"**Week Summary:** {total_rain:.2f} inches expected rain • Avg daily thirst (ET₀) {avg_et0:.2f} in/day")
            ...
            if p.get("companions"):
                ...
                for comp in p["companions"]:
                    if st.button(f"+ {comp}", key=f"comp_{idx}_{comp}"):
                        ...
                        if companion and companion["name"] not in [g["name"] for g in st.session_state.garden]:
